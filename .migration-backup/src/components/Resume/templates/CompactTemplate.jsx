// ============================================================
// The Data-Driven Pro — Numbers, Metrics, Success
// ============================================================
...
export default function DataDrivenProTemplate({ 
  data, 
  ...
  const config = {
    nameSize: s.name_size ? `${s.name_size}pt` : '18pt',
    headerSize: s.header_size ? `${s.header_size}pt` : '11pt',
    textSize: s.text_size ? `${s.text_size}pt` : '9pt',
    lineHeight: parseFloat(s.line_height || 1.3),
    marginTop: styles.marginTop ? `${styles.marginTop}mm` : '12mm',
    marginBottom: styles.marginBottom ? `${styles.marginBottom}mm` : '12mm',
    marginSide: styles.marginSides ? `${styles.marginSides}mm` : '15mm',
  };

  const bodyStyle = { fontSize: config.textSize, lineHeight: config.lineHeight, color: '#1F2937', fontFamily: isAr ? 'var(--global-font)' : '"Inter", sans-serif' };
  const headingStyle = { 
    color: accentColor, 
